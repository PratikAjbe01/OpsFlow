...
import { Sparkles, Save, Type, Hash, List, CheckSquare, Settings, Paintbrush, Trash2, Share2, Mail } from 'lucide-react';
...
const { accessToken } = useAppSelector((state) => state.auth);
  ...
  const { fields, theme, settings, selectedFieldId, isUnsaved } = useAppSelector((state) => state.builder);
  
  const [activeTab, setActiveTab] = useState<'properties' | 'design'>('design');
  ...
  useEffect(() => {
    if (form) {
      dispatch(setFields({ 
        ...
        theme: form.theme || {}, 
        settings: form.settings || { collectEmails: false, limitOneResponse: false } 
      }));
    }
  }, [form, dispatch]);
...
const getContainerStyles = () => ({
    ...
    borderWidth: theme.borderStyle === 'thin' ? '1px' : 
                 theme.borderStyle === 'thick' ? '3px' : 
                 theme.borderStyle === 'double' ? '4px' : '0px',
    borderStyle: theme.borderStyle === 'double' ? 'double' : 'solid',
    borderRadius: theme.borderRadius === 'none' ? '0' : 
                  theme.borderRadius === 'full' ? '24px' : `var(--radius-${theme.borderRadius})`,
  ...
  const radiusMap: any = { none: '0px', sm: '0.25rem', md: '0.5rem', lg: '0.75rem', full: '1.5rem' };

  if (isLoading) return <div className="p-10 flex items-center justify-center min-h-screen text-gray-500">Loading Builder...</div>;

  return (
    <div className="flex h-screen flex-col bg-gray-50">
      {/* Header */}
      <header className="flex h-16 items-center justify-between border-b bg-white px-6 shadow-sm z-10">
        <h1 className="text-xl font-bold text-gray-800">{form?.name || 'Untitled Form'}</h1>
        <div className="flex space-x-3 items-center">
            ...
            <div className="flex items-center space-x-4 mr-4 border-r pr-4 h-8">
                 <label className="flex items-center text-xs font-medium text-gray-600 cursor-pointer select-none hover:text-gray-900">
                    <input 
                        ...
                        onChange={(e) => dispatch(updateSettings({ collectEmails: e.target.checked }))}
                 ...
                 <label className="flex items-center text-xs font-medium text-gray-600 cursor-pointer select-none hover:text-gray-900">
                    <input 
                        ...
                        onChange={(e) => dispatch(updateSettings({ limitOneResponse: e.target.checked }))}
                        className="mr-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    Limit 1 Response
                 </label>
          ...
          <button onClick={() => setShowAI(true)} className="flex items-center rounded-md bg-purple-100 px-4 py-2 text-sm font-medium text-purple-700 hover:bg-purple-200 transition-colors">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 border-r bg-white p-4 overflow-y-auto">
             <h3 className="font-semibold text-gray-500 uppercase text-xs mb-4 tracking-wider">Toolbox</h3>
             <div className="space-y-3">
                 <ToolboxBtn icon={Type} label="Text Input" onClick={() => addManualField('text')} />
                 <ToolboxBtn icon={Hash} label="Number Input" onClick={() => addManualField('number')} />
                 <ToolboxBtn icon={List} label="Dropdown" onClick={() => addManualField('select')} />
                 <ToolboxBtn icon={CheckSquare} label="Checkbox" onClick={() => addManualField('checkbox')} />
                 <ToolboxBtn icon={Type} label="Text Area" onClick={() => addManualField('textarea')} />
             </div>
        </div>
        ...
        <div 
          ...
          onClick={() => { dispatch(selectField(null)); setActiveTab('design'); }}
        >
          <div 
          ...
          >
             <div className="space-y-6">
                {/* Form Title */}
                <h1 className="text-3xl font-bold border-b pb-4 mb-6" style={{ 
     ...
     color: theme.textColor || '#000000' 
                ...
                {fields.length === 0 ? (
                    <div className="text-center text-gray-900 py-20 opacity-40 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        <p className="text-sm">Click a tool on the left to add fields.</p>
                    </div>
                ) : (
                    fields.map((field: any) => (
                        <div 
                            key={field.id} 
                            onClick={(e) => { e.stopPropagation(); dispatch(selectField(field.id)); }}
                        ...
                        >
                            ...
                            <label className="block text-sm font-medium mb-1  pointer-events-none" style={{ color: theme.textColor }}>
  ...
  placeholder={field.placeholder || "Type your answer here..."} // Ensure placeholder text exists
  disabled 
/>
                                ) : field.type === 'select' ? (
                                    <select className="w-full rounded border text-black border-gray-300 p-2 bg-white opacity-80" disabled>
                                    ...
                                    <div className="flex items-center pt-2">
                                        ...
                                        <span className="ml-2 text-sm text-gray-500">Checkbox Option</span>
                                    </div>
                                ) : (
                                   <input 
  type={field.type} 
...
/>
                ...
                {fields.length > 0 && (
                    <div className="pt-4 border-t mt-6" style={{ borderColor: theme.borderColor }}>
                        <button 
                            className={`px-6 py-2.5 ${theme.btnColor===null?'text-black':'text-white'} font-medium shadow-sm hover:opacity-90 transition-opacity`}
                            style={{ 
        ...
        <div className="w-80 border-l bg-white flex flex-col h-full shadow-xl z-20">
             <div className="flex border-b">
                 <button 
                   ...
                   className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 ${activeTab === 'properties' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-500 hover:text-gray-700'}`}
                 >
                   <Settings className="w-4 h-4" /> Fields
                 </button>
                 <button 
                   onClick={() => setActiveTab('design')}
                   className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 ${activeTab === 'design' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-500 hover:text-gray-700'}`}
                 >
                   <Paintbrush className="w-4 h-4" /> Design
                 </button>
             </div>
             <div className="flex-1 overflow-y-auto">
                 {activeTab === 'properties' ? <PropertiesPanel /> : <DesignPanel />}
             </div>
        </div>
      ...
      {showShare && <ShareModal formId={id as string} onClose={() => setShowShare(false)} />}
    </div>
  );
}
